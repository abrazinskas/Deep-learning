...
class MLP(object):
    ...
    def __forward_pass(self, input):
        out = input
        # I'm adding 1 because the last layer is output layer
        for i in range(len(self.n_hidden)+1):
            W = self.weights[i]
            b = self.biases[i]
            out = tf.add(tf.matmul(out, W), b)
            # make sure we don't have non-linearities on the output layer
            out = tf.cond(tf.constant(i < len(self.n_hidden)), lambda: self.activation_fn(out), lambda: out)
            out = tf.cond(tf.constant(self.is_training), lambda: tf.nn.dropout(out, keep_prob=1-self.dropout_rate), lambda: out)
            tf.histogram_summary("out_"+str(i), out)
        return out
    ...
    def loss(self, logits, labels):
        ...
        if self.weight_regularizer is not None:
            loss += tf.add_n([self.weight_regularizer(W) for W in self.weights])
        ########################
        # END OF YOUR CODE    #
        #######################
