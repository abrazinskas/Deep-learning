...
class ELULayer(Layer):
    ...
    def backward(self, dout):
        ...
        x = self.cache['s_cur']
        out = x
        out[x<0] = x[x<0] + self.layer_params['alpha']
        out[x>=0] = 1
        dx = np.multiply(x, dout)
        ########################################################################################
        #                              END OF YOUR CODE                                        #
    ...
    def act(self, x):
        x[x<0] = self.layer_params['alpha'] * (np.exp(x[x<0]) - 1)
        return x
...
class SoftMaxLayer(Layer):
    ...
    def backward(self, dout):
        ...
        for i in range(x.shape[0]):
            # outer product
            dist = x[i]
            J = - np.outer(dist,dist)
            # set diagonal elements
            J[np.diag_indices_from(J)] = dist*(1 - dist)
            dx.append(np.dot(J, dout[i]))
            # J.append(A)
        # dx = np.tensordot(dout[i],J)
        ########################################################################################
        #                              END OF YOUR CODE                                        #
        ########################################################################################
