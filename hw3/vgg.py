...
from support import  new_fc_layer, flatten_layer
...
class VGG(object):
   ...
   in inference.
    ...
    def inference(self, x):
        ...
        Args:
          x: 4D float Tensor of features from VGG
        ...
        """
        with tf.variable_scope('VGG_feat_net'):
            ########################
            # PUT YOUR CODE HERE  #
            ########################
    ...
    def __forward_pass(self, x):
        ...
        with tf.variable_scope('flatten'):
            layer3, num_features = flatten_layer(x)
        # print(layer3)
        ...
        tf.histogram_summary(weights4.name, weights4)
        tf.histogram_summary(weights5.name, weights5)
        tf.histogram_summary(weights6.name, weights6)
