...
def create_dataset(source_data, num_tuples = 500, batch_size = 128, fraction_same = 0.2):
    ...
    for _ in range(num_tuples):
        dset.append(source_data.next_batch(batch_size=batch_size, fraction_same=fraction_same))
    ########################
    # END OF YOUR CODE    #
    ########################
    return dset
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fraction_same = 0.2):
    ...
    # PUT YOUR CODE HERE  #
    ########################
    pointer = self._index_in_epoch
    self._index_in_epoch += 1  # one anchor image per batch
    ...
    img1 = self._images[pointer]
    l1 = self._labels[pointer]

    # compute the number of pos and neg we need
    nr_pos = round(batch_size*fraction_same)
    nr_neg = batch_size - nr_pos

    x1 = np.array([img1], dtype="float32").repeat(batch_size, axis=0)
    x2 = []
    labels = []
    cur_nr_pos = 0
    cur_nr_neg = 0
    while len(labels) < batch_size:
        idx = np.random.choice(self._num_examples)
        # don't want to have same images
        if idx == pointer:
            continue
        img2 = self._images[idx]
        l2 = self._labels[idx]
        l = int(l2 == l1)
        # this line is for keeping the fraction
        if (l == 0 and cur_nr_neg >= nr_neg) or (l == 1 and cur_nr_pos >= nr_pos):
            continue
        x2.append(img2)
        labels.append(l)
        if l==0:
    ...
    # END OF YOUR CODE    #
    ########################
    return x1, np.array(x2, dtype="float32"), (np.array(labels)).reshape((-1, ))
